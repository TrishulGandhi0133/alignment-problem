...
export default function LobbyPage() {
  const { roomCode, myId, gameMode, players, isHost, startGame, error, clearError } = useGame();

  const activeCount = players.filter((p) => !p.isSpectator).length;
  const spectatorCount = players.filter((p) => p.isSpectator).length;
  const canStart = gameMode === "cricket" ? activeCount === 2 : players.length >= 4;
      ...
      <div className="card fade-in">
        ...
        <div className="mb-md">
          <div className="row mb-sm" style={{ justifyContent: "space-between" }}>
            ...
            <span className="muted" style={{ fontSize: "0.85rem" }}>
              {gameMode === "cricket" ? `${activeCount} active · ${spectatorCount} spectators` : `${players.length} / 10`}
            </span>
          </div>
          <div className="player-list">
            {players.map((p, i) => (
              <div key={p.id} className="player-item fade-in">
                ...
                {gameMode === "cricket" && p.teamSide && <span className="badge badge-you">Team {p.teamSide}</span>}
                {gameMode === "cricket" && p.isSpectator && <span className="badge badge-dead">SPECTATOR</span>}
              </div>
            ))}
          </div>
        </div>
        ...
        {!canStart && (
          <div className="alert alert-info mb-md">
            {gameMode === "cricket"
              ? "WAITING FOR PLAYERS — Cricket needs exactly 2 active players. Extra users join as spectators."
